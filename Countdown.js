...
var x = setInterval(function() {
  ...
  cookieTime.innerHTML = days + "d " + hours + "h "
  + minutes + "m " + seconds + "s";
    
  // If the count down is over, write some text 
  if (TimeDiff < 0) {
    clearInterval(x);
    cookieTime.innerHTML = "Merry Christmas, its time for the Secret Cookie Sweepstakes";
  }
  //If the day is 1 day before the countDownDate
  else if (days < 2 && days > 1){
  	TimeText.innerHTML ="Its Christmas Eve! One more day until the Secret Cookie Sweepstakes is here!";
    }
    //If the day is before the cut off date
    else if(days > 2){
      TimeText.innerHTML = "Make sure to enter in the Secret Cookie Sweepstakes for Christmas Eve!";
    }
    //Any other condition
    else{
